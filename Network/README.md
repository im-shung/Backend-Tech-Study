...
## SSL(Secure Sockets Layer), TLS(Transport Layer Sercurity)
- SSL/TLS의 역할
  - **암호화**: 도청되더라도 내용을 알 수 없도록 암호화한다.
  - **인증**: 올바른 대상인지 확인한다.
  - **조작 검사**: 내용이 조작되었는지 검출한다.
- SSL/TLS는 안전한 보안 채널을 형성해주는 역할을 수행하는 보안용 프로토콜이다.
- **어플리케이션 계층(HTTP)과 전송계층(TCP) 사이**에 위치해있다.
  - 세션계층 프로토콜이라는 건 아니다.
- HTTPS는 SSL/TLS에 HTTP를 얹어서 보안된 HTTP 통신을 하는 프로토콜이다. 
- 그래서 **SSL/TLS/는 다른 프로토콜까지 포함할 수 있고, HTTPS는 그저 SSL/TLS와 HTTP가 조합된 프로토콜만을 가리킨다.**
## HTTPS란
