...
<img src="../imgs/network-unicast.png">
...
- 어떤 PC가 유니캐스트 프레임을 뿌리게 되면, LAN 안의 모든 PC들에게 정보를 뿌리게 된다. (Shared 방식) 모든 PC들은 일단 프레임을 받아들여서 LAN카드의 자신의 Mac Address와 비교하고 불일치하는 경우 즉시 프레임을 버린다. 일치하는 경우 프레임을 CPU로 올려보낸다.
- 자신의 Mac Address와 불일치하는 경우 즉시 프레임을 버리기 때문에, PC의 CPU까지 영향을 주지 않아 PC의 성능이 저하되는 일은 없다.

## 멀티캐스트

<img src="../imgs/network-multicast.png">

예를 들어, 200명의 사용자가 있는 네트워크에서 150명에게만 같은 정보를 동시에 보내야 하는 상황이라고 가정해보자. 보내는 방법은 여러가지가 있다.
1. 유니캐스트 방식으로 150명이 주소로 하나씩 보내주기 → 트래픽 가중
2. 브로드캐스 방식으로 한 번에 모든 사용자에게 보내기 → 나머지 50명은 필요없는 데이터를 받아야함

이러한 문제들을 해결할 수 있는 통신 방식이 멀티캐스트이다.

멀티캐스트는
- 그룹에 속해있는 사람들에게만 선택적으로 보낸다.
- 하나의 데이터그램을 다중의 목적지(그룹)에게 복사하여 전달한다. 발신지가 하나의 패킷을 보내고, 네트워크(주로,라우터)쪽에서 이의 복사본을 여러 곳에 보낼 수 있도록 기능 확장을 도모하는 방향으로 나아가고 있다.
- 따라서 라우터와 스위치에서 멀티캐스트를 지원해야 한다. 지원하지 않으면 라우터는 멀티캐스트를 브로드캐스트 취급해 다 막아버린다. 
  - 라우터는 브로드캐스트를 막는 성질이 있다.
  - 스위치는 브로드캐스트처럼 모든 포트로 뿌려버린다.

## 브로드캐스트

<img src="../imgs/network-broadcast.png">

- 송신자가 포함된 LAN 안의 모든 네트워크 장비들에게 보내는 통신이다.
- 브로드 캐스트 주소: `FFFF.FFFF.FFFF`
- 이 주소를 가진 프레임이 오면 비록 자신의 Mac Address와 같지는 않지만 이 브로드캐스트 패킷을 CPU에게 보내고 CPU가 이 패킷을 처리한다.
- 브로드캐스트가 발생하는 경우는 다음과 같다.
  - 1. 처음 두 PC 간에 통신을 하는 경우, 상대편의 IP 주소는 알 수 있어도 상대편의 Mac Address는 알지 못한다. 이 경우에 상대의 Mac Address를 알아내기 위해 ARP(Address Resolution Prootocol) == Broadcast를 보낸다.
  - 2. 라우터끼리 정보를 교환하거나 다른 라우터를 찾을 때, 또 서버들이 자신이 어떤 서비스를 제공한다는 것을 알릴 때 등 여러 경우에 사용된다.


<hr>

출처
- [정보통신기술용어해설 - Multicast, Multicasting, 멀티캐스트, 멀티캐스팅, 멀티케스트, 멀티케스팅](http://www.ktword.co.kr/test/view/view.php?m_temp1=628)
- [유니캐스트, 브로드캐스트, 멀티캐스트, OSI 7 Layer 개념 정리](https://dltjrals2.github.io/network/network-concept-3/)
